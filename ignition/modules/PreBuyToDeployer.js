...
module.exports = buildModule("StremePreBuyETHtoDeployerModule", (m) => {
  const prebuy = m.contract("StremePreBuyETHtoDeployer", [process.env.PREBUY_TOKEN, process.env.PUBLIC_KEY]);
  console.log(`npx hardhat verify --network ${chain} ${prebuy.address} ${process.env.PREBUY_TOKEN} ${process.env.PUBLIC_KEY}`);
...
// npx hardhat ignition deploy ignition/modules/PreBuyToDeployer.js --network base --deployment-id prebuydep-base-two